...
for ep in range(100):
    for (x, y) in zip(X, Y):
        pre_y = perceptron.forward_compute(x)
        x1, x2 = x[0], x[1]
        ...
        perceptron.w1 = perceptron.w1 + lr * dw1  # 更新w1
        perceptron.w2 = perceptron.w2 + lr * dw2  # 更新w2
        perceptron.b = perceptron.b + lr * db  # 更新b

print(perceptron.w1,perceptron.w2,perceptron.b)
for (x, y) in zip(X, Y):  #测试结果
    t = 1 if perceptron.forward(x) > 0.5 else 0
    s = ''
    if t == y.item():
