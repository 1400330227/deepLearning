...
class OneModule(nn.Module):
    ...
    # n1 means the input channel and n2 means the output channel
    def __init__(self, n1, n2):
        super(OneModule, self).__init__()
        self.cnn = nn.Sequential(
            # n1=3, n2=64 (3, 160,240) -> (160-3+2*1)/1+1
            nn.Conv2d(n1, n2, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(n2),
            nn.ReLU(inplace=True),
            nn.Conv2d(n2, n2, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(n2),
            nn.ReLU(inplace=True),
        )
...
train_dataset = GetDataset(img_dir='../datasets/data/semantic-seg/train_imgs/',
                           mask_dir='../datasets/data/semantic-seg/train_masks/', flag='train')
...
def showTwoimgs(imgs, stitle='', rows=1, cols=2):
    ...
    plt.show()
unet_model = torch.load('unet_model.pt')  # unet_model10 是没有被覆盖的
unet_model.eval()
val_dataset = GetDataset(img_dir='../datasets/data/semantic-seg/val_imgs/',
                         mask_dir='../datasets/data/semantic-seg/val_masks/', flag='test')
